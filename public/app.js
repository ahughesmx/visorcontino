...
function renderLeads(leads) {
    ...
    leads.forEach(lead => {
        ...
        tr.onclick = () => openDetail(lead.id_registro);
        ...
        tr.innerHTML = `
            ...
            <td>${date}</td>
            <td>
                <button class="nav-item" style="padding: 0.3rem 0.6rem; margin:0;" onclick="event.stopPropagation(); openDetail(${lead.id_registro})">
                    <i data-lucide="eye"></i>
                </button>
            </td>
        `;
...
async function fetchActiveAgents() {
    try {
        ...
        availableAgents = agents.map(a => a.username);
    } catch (err) {
...
window.openUserModal = function (user = null) {
    ...
    try {
        ...
        if (!modalOverlay) {
            console.error('user-modal-overlay not found!');
